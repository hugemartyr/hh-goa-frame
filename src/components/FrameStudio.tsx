...
export function FrameStudio({ format, onFormatChange }: Props) {
  ...
  const addFiles = useCallback(async (files: FileList | File[]) => {
    const list = Array.from(files).filter((f) => /image|hei[cf]/i.test(f.type) || /\.(heic|heif)$/i.test(f.name));
    if (!list.length) {
          ...
          {busy ? (
            ...
            <>
              ...
              <p className="max-w-sm text-sm text-muted-foreground">
                JPG, PNG or iPhone HEIC. We auto-crop around your face — no manual cropping,
